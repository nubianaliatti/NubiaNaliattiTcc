...
def get_closest_hourly_row(index, hourly_dataframe, hour_column, df_running_metrics):  
...
def searchHistoricWeather(index, data_column, hour_column, df_running_metrics):
	...
	get_closest_hourly_row(index, hourly_dataframe, hour_column, df_running_metrics)
	#print(hourly_dataframe)
	
         
def creating_data_frame(pasta):	
	
	df = pd.read_csv(f'./eliza/extracted/{pasta}/com.samsung.shealth.exercise.csv', delimiter=',')#, skiprows=2)
	print(df.info)
	...
	'''
	df_running_metrics = df[['live_data_internal', 'total_calorie', 'sensing_status', 'heart_rate_sample_count', 
							...
							'com.samsung.health.exercise.live_data', 'com.samsung.health.exercise.mean_speed', 'com.samsung.health.exercise.end_time',
	...
	for index, row in df_running_metrics.iterrows():
		...
		searchHistoricWeather(index, df_running_metrics['data'].loc[index], df_running_metrics['hora'].loc[index], df_running_metrics)

	
	df_sleep = pd.read_csv(f'./eliza/extracted/{pasta}/com.samsung.shealth.sleep.csv', delimiter=',')#, skiprows=2)
	...
	df_recovery = pd.read_csv(f'./eliza/extracted/{pasta}/com.samsung.shealth.exercise.recovery_heart_rate.csv', delimiter=",")#, skiprows=2)
...
'''
creating_data_frame('samsunghealth_pedroalmeidavrb_20250106153699')