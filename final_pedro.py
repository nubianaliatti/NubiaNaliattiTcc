...
def get_closest_hourly_row(index, hourly_dataframe, hour_column):  
	...
	df_running_metrics['temperature_2m'].loc[index] = filtered_row['temperature_2m'].values[0]
	df_running_metrics['relative_humidity_2m'].loc[index] = filtered_row['relative_humidity_2m'].values[0]
	df_running_metrics['wind_speed_10m'].loc[index] = filtered_row['wind_speed_10m'].values[0]
...
df = pd.read_csv('com.samsung.shealth.exercise.consolidado.csv', delimiter=';', skiprows=2)
...
df_merged.to_csv('novosDadosTeste.csv', index=False,encoding="utf-8-sig")
